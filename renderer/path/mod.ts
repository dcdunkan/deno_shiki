...
import { load, parse } from "https://esm.sh/opentype.js@1.3.4";
...
export async function getPathRenderer(options: PathRendererOptions) {
  const font = typeof options.fontFile === "string"
    ? await load(options.fontFile)
    : parse(options.fontFile);

  if (!font.supported) {
    throw new Error(`The font "${options.fontFile}" is not supported`);
  }
  ...
  return {
    renderToSVG(lines: IThemedToken[][], { bg } = { bg: _bg }) {
       ...
       */
      const bgWidth = longestLineWidth +
        bgSideCharPadding * 2 * singleCharWidth;
      /**
